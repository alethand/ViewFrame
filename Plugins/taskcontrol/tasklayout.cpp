...
#include <core/protocol/commonprotocolparse.h>
 ...
 */
void TaskLayoutParse::releaseContainer(Core::Datastruct::Container *container)
{
    if(container->childContainer.size() == 0){
        ...
        std::for_each(container->childContainer.begin(),container->childContainer.end(),[&](Core::Datastruct::Container* cter){
       ...
       if(domElem->tagName().toLower() == nodeInfo.window)
       {
           ...
           for(int i = 0; i < childList.size();i++){
               if(childList.at(i).toElement().tagName().toLower() == nodeInfo.widget){
                    Core::Datastruct::Container * childContainer = new Core::Datastruct::Container();
 ...
 */
void TaskLayoutParse::parseContainer(QDomNode &node, Core::Datastruct::Container *element)
{
    QDomNodeList childList = node.childNodes();
    if(childList.size() == 2){
        Core::Datastruct::NodeInfo nodeInfo;
        ...
        if(elem.attribute(nodeInfo.type).toLower() == nodeInfo.widget){
            ...
            element->continer.type = Core::Datastruct::Widget;
        }
            ...
            element->continer.type = Core::Datastruct::Groupbox;
        }
 ...
 */
void TaskLayoutParse::parseLayout(QDomNode &node,Core::Datastruct::Container *element)
{
    Core::Datastruct::NodeInfo nodeInfo;
 ...
 */
void TaskLayoutParse::parseItems(QDomNode &node,Core::Datastruct::Container *element)
{
    Core::Datastruct::NodeInfo nodeInfo;
 ...
 * @return
 */
Core::Datastruct::Field *TaskLayoutParse::parseAttributes(QDomNode &node)
{
     QDomElement * domElem = NULL;
     bool hasAtttribut = true;
     Core::Datastruct::Field * newItem = new Core::Datastruct::Field();
     ...
     while(!node.isNull() && hasAtttribut)
     {
         ...
         else if(nodeInfo.itemComboxList == domElem->tagName()&&newItem->type == Core::Datastruct::ControlType::ComboBox
                 &&newItem->widget != NULL)
         {
             QDomNodeList nodeList = node.childNodes();
             for(int i = 0;i < nodeList.count();i++)
 ...
 */
void TaskLayoutParse::parseType(QDomNode &node, Core::Datastruct::PubHead *element)
{
    QDomElement *domElem = NULL;
    domElem = &(node.toElement());

    Core::Datastruct::WidgetType wtype;

    int fixedHeight = 24;

    if(domElem->text()== wtype.combox) {
        element->type = Core::Datastruct::ControlType::ComboBox;
 ...
 */
void TaskLayoutParse::parseItem(QDomNode &node, Core::Datastruct::Container *element)
{
    Core::Datastruct::Field * newItem = parseAttributes(node);
    if(NULL != newItem )
    {
 ...
 * @param[in] elem  容器
 */
void TaskLayoutParse::generateWidget(Core::Datastruct::Container * container)
{
    ...
    if(container->childContainer.size() == 0){
        ...
        {
            Core::Datastruct::Field *temp = container->fileds.at(i);
            switch(temp->type)
            {
 ...
 */
Core::Datastruct::Layout TaskLayoutParse::parseLayout(QString layoutText)
{
    if(layoutText == "vertical")
        return Core::Datastruct::Vertical;
    else if(layoutText == "horizonal")
        return Core::Datastruct::Horizonal;
    else if(layoutText == "grid")
        return Core::Datastruct::Grid;
    return Core::Datastruct::None;
}
