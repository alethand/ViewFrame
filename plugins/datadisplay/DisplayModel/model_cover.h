...
public:
    ...
    Model_Cover(DataManager *subject,Core::Data_Word *settings,bool openFilter = false);


    virtual QVariant headerData(int section, Qt::Orientation orientation,int role) const;
    virtual void getNewestInfo(Core::NetParse::Protocol* info);
...
private:
    ...
    struct CoverData{
        CoverData();
        Core::NetParse::Protocol *pProtocol;    /*! 截获协议*/
        QTime         time;         /*! 截获时间*/
        unsigned int  times;        /*! 截获次数*/
    };

    Core::Data_Word *mField_Appointed;                    //指定字段
