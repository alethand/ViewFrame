...
{
public:
    explicit DataManager(const Core::Datastruct::BaseProtocol *protocol);

    /*! @warning 此时默认Datastruct::ParsedResult 为一条网络数据报文 */
    void handleNetInfo(Core::NetParse::Protocol *netInfo);            ///处理网络数据
    void register_DataObserver(Observer *obj);                                  ///注册成为观察者

    const QList<Core::NetParse::Protocol*>*                           ///获取所有历史数据
...
private:
    QList<Core::NetParse::Protocol*> originData;    /*! 原始数据-随时间递增*/
    QList<Observer*> observers;                               /*! 观察者集合 */
    const Core::Datastruct::BaseProtocol *mProtocol;                /*! 解析的协议   */
};
...
public:
    ...
    virtual void getNewestInfo(Core::NetParse::Protocol* info)=0;    ///获取最新信息
...
protected:
    ...
    QList<Core::Data_Elem*>    mNeedsDisplayData;             /*! 需要进行显示的数据-不是全部数据 */
