    ...
    for(int i=0;i<row;i++)
        ...
        {                                                      //计算每张图片的边长，zoomnum为放大倍数
            ...
            if(j+pic_x>=0)                                         //判断列数的范围
            {
                 /*if(j+pic_x>=(Maxnum-1))
                 {
                      QString temp=QString("%1\\地图\\谷歌卫星地图\\Zoom_%2\\Tile_%3x%4.Png").arg(zoom).arg(zoom).arg((j+pic_x)%Maxnum).arg(i+pic_y); //列数超过总列数，循环从0算起
                      imagename = temp;
                 }
                  else
                 {
                     QString temp=QString("%1\\地图\\谷歌卫星地图\\Zoom_%2\\Tile_%3x%4.Png").arg(zoom).arg(zoom).arg(j+pic_x).arg(i+pic_y);
                     imagename = temp;
                 }*/
